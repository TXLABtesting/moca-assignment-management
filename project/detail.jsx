// Assignment Detail — overview + approval timeline + documents + audit log + comments.
// Approver actions: Approve / Request changes / Reject (with required comment for rejection).

const { useState: useStateDet, useMemo: useMemoDet } = React;

function Detail({ id, onNavigate }) {
  const { t, lang } = useI18n();
  const toast = useToast();
  const [tab, setTab] = useStateDet('overview');
  const [showReject, setShowReject] = useStateDet(false);
  const [showApprove, setShowApprove] = useStateDet(false);
  const [rejectReason, setRejectReason] = useStateDet('');
  const [comment, setComment] = useStateDet('');

  const assignment = useMemoDet(() =>
    window.ASSIGNMENTS.find(a => a.id === id) || window.ASSIGNMENTS.find(a => a.status === 'pending'),
    [id]);

  if (!assignment) return <div className="empty">No assignment.</div>;

  const emp = window.GET_EMPLOYEE(assignment.employeeId);
  const fromEnt = window.GET_ENTITY(assignment.fromEntity);
  const toEnt = window.GET_ENTITY(assignment.toEntity);
  const gradeObj = window.GET_GRADE(assignment.grade);
  const dur = window.daysBetween(assignment.startDate, assignment.endDate);
  const daysLeft = window.daysBetween(new Date(), assignment.endDate);
  ...
  const handleApprove = () => {
    ...
    toast.push(t('toast_approved'), 'success');
  };
  const handleReject = () => {
    ...
    <>
      ...
      <div className="page-header">
        ...
        <div className="page-header-row">
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div className="page-subtitle tabular">{assignment.id} · {lang === 'ar' ? 'تم الإنشاء' : 'Created'} {window.formatDate(assignment.updatedAt, lang)}</div>
          </div>
          <div className="page-actions">
            {isPending && (
              <>
                ...
                <Button variant="primary" icon="check" onClick={() => setShowApprove(true)}>{t('approve')}</Button>
              </>
            )}
            {isActive && (
              <>
      ...
      {isExpiringSoon && (
        <div className="mb-5">
          <Alert type="warning" title={lang === 'ar' ? `تنتهي هذه المهمة خلال ${daysLeft} يوماً` : `This assignment expires in ${daysLeft} days`}>
            {lang === 'ar' ? 'يُرسل النظام تذكيرات تلقائية في 45 و30 و15 يوماً. ابدأ التمديد قبل تاريخ الانتهاء.' : 'Auto-reminders are set for 45, 30, and 15 days. Start the extension before the end date.'}
      ...
      <div className="with-rail">
        <div>
          ...
          {tab === 'timeline' && <TimelineTab comment={comment} setComment={setComment} />}
        ...
        <div className="rail flex-col" style={{ gap: 'var(--space-4)' }}>
          ...
          <NextActionsRail assignment={assignment} isPending={isPending} isActive={isActive} onNavigate={onNavigate} onApprove={() => setShowApprove(true)} onReject={() => setShowReject(true)} />
        </div>
      </div>
      ...
      <Modal open={showReject} onClose={() => setShowReject(false)}
        ...
        <Field label={lang === 'ar' ? 'سبب الرفض' : 'Reason for rejection'} required className="mt-3"
          hint={lang === 'ar' ? 'سيرى مقدم الطلب هذه الرسالة. كن واضحاً ومحترماً.' : 'The submitter will see this message. Be specific and respectful.'}>
      ...
      <Modal open={showApprove} onClose={() => setShowApprove(false)}
        title={t('approve') + ' — ' + assignment.id}
        footer={<>
          <Button variant="ghost" onClick={() => setShowApprove(false)}>{t('cancel')}</Button>
          <Button variant="primary" icon="check" onClick={handleApprove}>{t('confirm')} {t('approve')}</Button>
        </>}>
        <div className="text-sm" style={{ marginBottom: 'var(--space-3)' }}>
          {lang === 'ar' ? 'سيتم تمرير الطلب إلى الخطوة التالية في سلسلة الاعتماد:' : 'This will move the request to the next step in the approval chain:'}
        </div>
        <div className="flex items-center gap-3 mb-3" style={{ padding: '12px', background: 'var(--color-success-50)', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-success-100)' }}>
          <Avatar name={lang === 'ar' ? emp.ar : emp.en} />
          <div>
            <div className="text-strong">{lang === 'ar' ? emp.ar : emp.en}</div>
            <div className="text-sm">{lang === 'ar' ? fromEnt.ar : fromEnt.en} → {lang === 'ar' ? toEnt.ar : toEnt.en}</div>
          </div>
        </div>
...
function OverviewTab({ assignment, emp, fromEnt, toEnt, gradeObj, dur }) {
  const { t, lang } = useI18n();
  const [showProfile, setShowProfile] = useStateDet(false);
    ...
    <div className="flex-col" style={{ gap: 'var(--space-5)' }}>
      <Card title={t('step_employee')}>
        <div className="flex items-center gap-4">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="text-sm text-muted">{lang === 'ar' ? fromEnt.ar : fromEnt.en} · {t('grade')} {emp.grade} · {window.formatAED(window.GET_GRADE(emp.grade).basic, lang)}</div>
          </div>
          <Button size="sm" variant="secondary" icon="eye" onClick={() => setShowProfile(true)}>{lang === 'ar' ? 'الملف الشخصي' : 'View profile'}</Button>
        </div>
      </Card>
      ...
      <Card title={t('step_details')}>
        <div className="dl-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-5)' }}>
          ...
          <Datum label={t('start_date')} value={window.formatDate(assignment.startDate, lang)} />
          <Datum label={t('end_date')} value={window.formatDate(assignment.endDate, lang)} />
      ...
      <Card title={t('compensation_title')}>
        <div className="dl-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--space-5)' }}>
          <Datum label={t('salary_basic')} value={window.formatAED(assignment.salaryBasic, lang)} />
          <Datum label={t('allowance_amount')} value={assignment.allowance > 0 ? `${window.formatAED(assignment.allowance, lang)} (${allowancePct}%)` : (lang === 'ar' ? 'لا يوجد' : 'None')} />
          <Datum label={t('paying_entity')} value={lang === 'ar' ? window.GET_ENTITY(assignment.payingEntity).ar : window.GET_ENTITY(assignment.payingEntity).en} />
          <Datum label={lang === 'ar' ? 'إجمالي الشهري' : 'Monthly total'}
            value={window.formatAED(assignment.salaryBasic + (assignment.allowance || 0), lang)} />
          <Datum label={lang === 'ar' ? 'إجمالي عقد المهمة' : 'Total assignment cost'}
            value={window.formatAED(Math.round((assignment.salaryBasic + (assignment.allowance || 0)) * dur / 30), lang)} />
      ...
      <Modal open={showProfile} onClose={() => setShowProfile(false)}
        ...
        <div className="dl-grid" style={{ gridTemplateColumns: 'repeat(2, 1fr)', gap: 'var(--space-4)' }}>
          ...
          <Datum label={t('basic_salary')} value={window.formatAED(window.GET_GRADE(emp.grade).basic, lang)} />
          <Datum label={t('join_date')} value={window.formatDate(emp.joined, lang)} />
        </div>
      </Modal>
    </div>
  );
...
// ============================================================
// Timeline tab — approval chain
// ============================================================
function TimelineTab({ comment, setComment }) {
  const { t, lang } = useI18n();
  const toast = useToast();
  const items = window.SAMPLE_TIMELINE;
          ...
          <div key={i} className={`timeline-item ${item.status}`}>
            ...
            <div className="timeline-body">
              <div className="timeline-head">
                ...
                {item.timestamp && <span className="timeline-time tabular">{window.formatDateTime(item.timestamp, lang)}</span>}
              ...
              {item.status === 'active' && (
                <div className="mt-3" style={{ display: 'flex', gap: 8 }}>
                  <Button size="sm" variant="primary" icon="check">{t('approve')}</Button>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div className="text-xs text-muted">
                ...
                {lang === 'ar' ? 'يظهر التعليق لجميع المعتمدين والمشاركين في الطلب.' : 'Comments are visible to all approvers and stakeholders.'}
              </div>
        ...
        {docs.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-3)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="text-xs text-muted">
                {window.formatDate(d.date, lang)} · {lang === 'ar' ? d.author_ar : d.author_en} · {d.size}
              </div>
            </div>
...
function HistoryTab() {
  const { t, lang } = useI18n();
  const entries = [
    { time: '2026-05-19T08:42:00', actor_en: 'Dr. Saeed Al Tayer', actor_ar: 'د. سعيد الطاير',
      action_en: 'Approved at Line Manager level', action_ar: 'اعتمد على مستوى المدير المباشر', icon: 'check', color: 'var(--color-success-600)' },
    { time: '2026-05-18T16:21:00', actor_en: 'System', actor_ar: 'النظام',
      action_en: 'Routed to Line Manager (Dr. Saeed Al Tayer)', action_ar: 'تم التوجيه إلى المدير المباشر (د. سعيد الطاير)', icon: 'send', color: 'var(--color-info-600)' },
    { time: '2026-05-18T09:14:00', actor_en: 'Mariam Al Mansoori', actor_ar: 'مريم المنصوري',
      action_en: 'Submitted for approval', action_ar: 'قدّم للاعتماد', icon: 'send', color: 'var(--color-info-600)' },
    { time: '2026-05-18T09:11:00', actor_en: 'Mariam Al Mansoori', actor_ar: 'مريم المنصوري',
      action_en: 'Uploaded employee consent letter', action_ar: 'رفع خطاب موافقة الموظف', icon: 'upload', color: 'var(--color-ink-600)' },
    { time: '2026-05-18T08:47:00', actor_en: 'Mariam Al Mansoori', actor_ar: 'مريم المنصوري',
      action_en: 'Created draft', action_ar: 'أنشأ مسودة', icon: 'edit', color: 'var(--color-ink-600)' },
  ];
  return (
      ...
      <div className="flex-col" style={{ gap: 0 }}>
        {entries.map((e, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '32px 1fr auto', gap: 'var(--space-3)', padding: '12px 0', borderBottom: i < entries.length - 1 ? '1px solid var(--border-subtle)' : 0, alignItems: 'center' }}>
            ...
            <div className="text-xs text-muted tabular" style={{ whiteSpace: 'nowrap' }}>{window.formatDateTime(e.time, lang)}</div>
          </div>
        ))}
      </div>
    </Card>
  ...
  return (
    <Card title={lang === 'ar' ? 'ملخص' : 'Summary'}>
      <div className="flex-col" style={{ gap: 'var(--space-3)' }}>
        <SummaryItem label={lang === 'ar' ? 'من' : 'From'} value={lang === 'ar' ? fromEnt.ar : fromEnt.en} />
        <SummaryItem label={lang === 'ar' ? 'إلى' : 'To'} value={lang === 'ar' ? toEnt.ar : toEnt.en} highlight />
        <SummaryItem label={t('duration')} value={`${dur} ${t('days')}`} />
        <SummaryItem label={t('grade')} value={assignment.grade} mono />
        <SummaryItem label={lang === 'ar' ? 'إجمالي شهري' : 'Monthly comp.'} value={window.formatAED(assignment.salaryBasic + (assignment.allowance || 0), lang)} />
...
function SummaryItem({ label, value, mono, highlight }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}>
      ...
      <span style={{ textAlign: 'end', fontSize: 'var(--text-sm)', fontWeight: highlight ? 600 : 500, color: highlight ? 'var(--color-gold-800)' : 'var(--color-ink-900)', fontVariantNumeric: mono ? 'tabular-nums' : 'normal' }}>{value}</span>
    </div>
  );
}

function NextActionsRail({ assignment, isPending, isActive, onNavigate, onApprove, onReject }) {
  const { t, lang } = useI18n();
  if (isPending) {
    return (
      <Card title={lang === 'ar' ? 'إجراءاتك' : 'Your actions'}>
        ...
        <div className="flex-col mt-3" style={{ gap: 8 }}>
          <Button variant="primary" icon="check" onClick={onApprove} style={{ width: '100%' }}>{t('approve')}</Button>
