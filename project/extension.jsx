// Extension Request — quick flow built on top of an existing assignment.
// Surfaces Minister-approval rule for loan/borrowing types live.

const { useState: useStateE, useMemo: useMemoE } = React;

function Extension({ id, onNavigate }) {
  const { t, lang } = useI18n();
  const toast = useToast();
  const assignment = useMemoE(() =>
    window.ASSIGNMENTS.find(a => a.id === id) ||
    window.ASSIGNMENTS.find(a => a.status === 'active'), [id]);
  ...
  // Default proposed end = current end + 90 days (within 180-day cap)
  const defaultNewEnd = useMemoE(() => {
  ...
  // Acting & secondment: extend in whole months, capped at 6 months (180 days).
  // Loan & borrowing: any period, but HR must attach the Minister's signed decree.
  ...
  const proposedEnd = isCapped ? window.addMonths(assignment.endDate, form.months) : form.newEnd;
  const additionalDays = window.daysBetween(assignment.endDate, proposedEnd);
        ...
        <div className="flex-col" style={{ gap: 'var(--space-5)' }}>
          ...
          <Card title={lang === 'ar' ? 'المهمة الحالية' : 'Current assignment'}>
            <div className="flex items-center gap-4 mb-4">
              ...
              <div style={{ flex: 1 }}>
                <div className="flex items-center gap-2 mb-1">
                  <span style={{ fontSize: 'var(--text-md)', fontWeight: 600 }}>{lang === 'ar' ? emp.ar : emp.en}</span>
            ...
            <div className="dl-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
              ...
              <Datum label={t('ext_current_end')} value={window.formatDate(assignment.endDate, lang)} />
          ...
          <Card title={lang === 'ar' ? 'تفاصيل التمديد' : 'Extension details'}>
            ...
            <div className="grid-3 mb-4">
              <Field label={t('ext_current_end')}>
                <Input value={window.formatDate(assignment.endDate, lang)} readOnly style={{ background: 'var(--bg-muted)' }} />
              </Field>
              {isCapped ? (
              ...
              <Field label={lang === 'ar' ? 'تاريخ النهاية الجديد' : 'New end date'}>
                <Input value={window.formatDate(proposedEnd, lang)} readOnly style={{ background: 'var(--bg-muted)', fontWeight: 600 }} />
            ...
            <div className="mb-4" style={{ padding: 'var(--space-4)', background: 'var(--bg-muted)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-subtle)' }}>
              ...
              <div style={{ position: 'relative', height: 40, marginTop: 4 }}>
                ...
                {/* Markers */}
                <div style={{ position: 'absolute', top: 0, left: 0, fontSize: 11, color: 'var(--color-ink-700)' }}>{window.formatDate(assignment.startDate, lang)}</div>
                <div style={{ position: 'absolute', top: 0, left: '60%', fontSize: 11, color: 'var(--color-ink-700)', transform: 'translateX(-50%)' }}>{window.formatDate(assignment.endDate, lang)}</div>
                <div style={{ position: 'absolute', top: 0, right: 0, fontSize: 11, color: exceeds180 ? 'var(--color-danger-600)' : 'var(--color-success-700)', fontWeight: 500 }}>{window.formatDate(proposedEnd, lang)}</div>
              </div>
            ...
            <div className="mt-4 flex-col" style={{ gap: 'var(--space-3)' }}>
              <label className="checkbox">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted" style={{ marginTop: 2 }}>{window.formatAED(assignment.allowance, lang)}/{lang === 'ar' ? 'شهر' : 'mo'}</div>
                </div>
              </label>
              <label className="checkbox">
        ...
        <div className="rail flex-col" style={{ gap: 'var(--space-4)' }}>
          ...
          <Card title={lang === 'ar' ? 'مسار الاعتماد' : 'Approval path'}>
            <div className="flex-col" style={{ gap: 10 }}>
              <ApprovalStep num={1} label={t('approver_entity_head')} active />
...
Object.assign(window, { Extension });
