...
// All Tailwind/DLS-aligned. No external icon library — inline strokes for full control + RTL.
...
// Button
// ============================================================
function Button({ variant = 'secondary', size, icon, iconAfter, children, className = '', ...rest }) {
...
function Field({ label, hint, error, required, optional, children, className = '' }) {
  ...
  return (
    <div className={`field ${error ? 'has-error' : ''} ${className}`}>
      ...
      {hint && !error && <div className="field-hint" data-comment-anchor="0b59edb1fd-div-97-26">{hint}</div>}
    </div>);
...
function InputWithIcon({ icon, suffix, ...rest }) {
  return (
...
// Searchable dropdown that also accepts a free-typed value not in the list.
// options: [{ value, label }]. value may be an option value OR a custom string.
...
function Avatar({ name, size = 'md' }) {
  ...
  return (
    <div className={`avatar avatar-${size}`} style={{ background: bg }}>{initials.toUpperCase()}</div>);
...
// Alert
// ============================================================
function Alert({ type = 'info', title, children, icon }) {
  ...
  return (
    <div className={`alert alert-${type}`}>
      ...
      <div className="alert-body">
        ...
        <div data-comment-anchor="bd49643f2f-div-166-9">{children}</div>
      </div>
    </div>);
...
// Card
// ============================================================
function Card({ title, subtitle, actions, children, footer, className = '', noBody = false }) {
  return (
...
Object.assign(window, {
  Icon, Button, Field, Input, Textarea, Select, InputWithIcon, Combobox,
  StatusBadge, TypeBadge, Avatar, Alert, Card, Modal,
