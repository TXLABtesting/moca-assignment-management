...
import { useI18n, Button, Card, StatCard } from './components';
import { ASSIGNMENTS, GET_ENTITY, daysBetween } from './data';
...
const POLICY_CONTENT = {
  ...
  sla: [
    { en: 'Automatic notification 45 days before any assignment ends — sent to: HR Specialist.', ar: 'إشعار تلقائي قبل 45 يوماً من انتهاء أي تكليف — يُرسل إلى: أخصائي الموارد البشرية.' },
    { en: 'Automatic notification 30 days before — sent to: HR Specialist and the relevant final approver.', ar: 'إشعار تلقائي قبل 30 يوماً — يُرسل إلى: أخصائي الموارد البشرية والمعتمد النهائي.' },
    { en: 'Automatic notification 15 days before — sent to: HR Specialist, final approver, and the employee concerned.', ar: 'إشعار تلقائي قبل 15 يوماً — يُرسل إلى: أخصائي الموارد البشرية والمعتمد النهائي والموظف المعني.' },
...
export function Policies({ onNavigate }) {
...
export function Reports({ onNavigate }) {
  ...
  const byEntity = useMemo(() => {
    ...
    return Object.keys(map)
      .map(id => ({ key: id, label: lang === 'ar' ? GET_ENTITY(id).ar : GET_ENTITY(id).en, count: map[id] }))
